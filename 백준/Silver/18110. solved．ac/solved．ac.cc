...
/*
	...
	지금까지는 다양한 문제를 풀어 보고 싶더라도 난이도를 가늠하기 어려워 
	무슨 문제를 풀어야 할지 판단하기 곤란했기 때문에 solved.ac가 만들어졌다. 
...
int main(void) {
	...
	for (int i = 0; i < tCase; i++) {
		cin >> tmp;
		evaluation.push_back(tmp);
	}

	//가장 낮은 평가, 가장 높은 평가 제거하기 위해 정렬
	sort(evaluation.begin(), evaluation.end());

	doubleLimit = evaluation.size() * 0.15;
	doubleLimit = round(doubleLimit);

	//cout <<'\n' << doubleLimit << '\n';

	//for (int i = 0; i < (int)doubleLimit; i++) {
	//	evaluation.pop_back();
	//	evaluation.erase(evaluation.begin());
	//}
	...
	//cout << sum<<" "<<evaluation.size() << '\n';

	for (int i = doubleLimit; i < evaluation.size()-doubleLimit; i++) {
		//cout << evaluation[i] << '\n';
		sum += evaluation[i];
	}
	...
	if (tCase == 0) {
		...
		average = round((double)sum / (evaluation.size()-doubleLimit*2));
